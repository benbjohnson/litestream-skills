...
Enable AI tool integration via Model Context Protocol:

```yaml
mcp-addr: "localhost:8080"    # MCP server address
```
...
Enable Virtual File System for read replicas:
...
dbs:
  - path: /data/app.db
    replica:
      ...
      vfs:
        enabled: true
        poll-interval: 1s       # Check for remote changes
        cache-size: 10MB        # LRU page cache size
        write: false            # Enable write support
        write-sync-interval: 1s # Sync interval for writes
```

See [VFS Support](../concepts/vfs.md) for details.
