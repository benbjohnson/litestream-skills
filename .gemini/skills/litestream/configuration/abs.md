...
When running on Azure infrastructure, use managed identity for secure, keyless authentication:
...
dbs:
  - path: /data/app.db
    replica:
      ...
      # No account-key needed - uses Azure Default Credential Chain
```

When `account-key` is omitted, Litestream uses Azure's default credential chain:
