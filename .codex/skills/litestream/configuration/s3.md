...
| `metadata-concurrency` | 50 | Parallel HeadObject calls for timestamp restore |
...
For multi-instance deployments, S3 supports distributed leasing using conditional writes to prevent conflicts:
...
replica:
  ...
  lease:
    enabled: true
    ttl: 30s           # Lease time-to-live
    renew-interval: 10s # How often to renew lease
```

This ensures only one Litestream instance writes to the replica at a time, preventing data corruption in high-availability setups.

### Metadata Concurrency

For timestamp-based restore with large backup histories, increase parallel metadata fetches:

```yaml
replica:
  url: s3://bucket/path
  metadata-concurrency: 100  # Default: 50
```

This speeds up point-in-time recovery by fetching LTX file timestamps in parallel.
